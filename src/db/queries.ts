...
): Pick<DbRunRow, "id" | "query" | "agent_count" | "status" | "pipeline_state" | "created_at"> {
  const now = Date.now();
  return {
    id: nanoid(),
    query: input.query,
...
function buildRunPatch(
  patch: RunStatusPatch,
): { fields: string[]; values: Array<string | number | null> } {
  const fields: string[] = [];
...
export function getRun(runId: string): RunRecord | null {
  const db = getDatabase();
  const row = db.prepare(`${RUNS_SELECT} WHERE id = ?`).get(runId) as DbRunRow | null;
...
export function addTokenUsage(runId: string, promptTokens: number, completionTokens: number): void {
  const db = getDatabase();
  db.prepare(
    `
    UPDATE runs
