...
function renderTokens(
	...
	const total = Math.max(
		0,
		Math.round(totalPromptTokens + totalCompletionTokens),
	);
...
export class HydraUI {
	...
	constructor(options: HydraUIOptions) {
		...
		this.#totalDebateRounds = Math.max(
			1,
			Math.trunc(options.totalDebateRounds),
		);
	}
		...
		if (event.type === "agent-progress") {
			...
			if (newPhase === "debating") {
				const isNewRoundByCompletion =
					event.completedAgents < this.#progressCompleted ||
					this.#debateRoundComplete;
			...
			if (
				newPhase === "debating" &&
				event.completedAgents >= event.totalAgents
			) {
	...
	#syncAgentListFromDb(): void {
		...
		if (!this.#runId) {
			this.#totalSearches = this.#agentEntries.reduce(
				(total, entry) => total + entry.searchCount,
				0,
			);
	...
	#refreshAgentRows(nowMs: number): void {
		...
		const rowCount = Math.min(
			this.#agentRows.length,
			this.#agentEntries.length,
		);
		for (let index = 0; index < rowCount; index += 1) {
			this.#agentRows[index].content = formatAgentLine(
				this.#agentEntries[index],
				nowMs,
			);
		}
	}
	...
	#refresh(nowMs = Date.now()): void {
		...
		const totalForDisplay = Math.max(
			1,
			this.#progressTotal || this.#totalAgents || 1,
		);
		...
		const eta =
			this.#status === "complete"
				? "0s"
				: this.#etaEstimator.estimate(remaining, this.#concurrency);
		const elapsed = formatDuration(
			this.#status === "decomposing"
				? this.#estimatedMs
				: Math.max(0, nowMs - this.#runCreatedAt),
		);
		...
		const errors = this.#agentEntries.filter(
	...
	#buildProgressBar(completed: number, total: number): string {
		const width = clamp(
			this.#renderer ? this.#renderer.width - 30 : 24,
			12,
			64,
		);
