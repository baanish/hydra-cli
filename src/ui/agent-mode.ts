...
import type { PipelineEvent, RunStatus } from "../types.js";
...
function phaseForAgentEvent(
	phase: "decompose" | "research" | "debate" | "synthesis",
): RunStatus {
	if (phase === "decompose") {
		return "decomposing";
	}
...
function formatAgentModeProgress(
	state: AgentModeState,
	eventPhase: RunStatus,
...
export function setAgentModeDebateRounds(totalDebateRounds: number): void {
	...
	agentModeState.totalDebateRounds =
		Number.isFinite(parsed) && parsed > 0 ? parsed : 1;
}
...
export function emitAgentProgress(event: PipelineEvent): void {
	if (event.type === "run-created") {
		...
		writeAgentModeLine(
			`${formatAgentModeProgress(agentModeState, "decomposing", false)} | runId=${agentModeState.runId}`,
		);
		return;
	}
	...
	if (event.type === "agent-progress") {
		...
		if (phaseStatus === "debating") {
			const isNewRoundByCompletion =
				event.completedAgents < agentModeState.completedAgents ||
				agentModeState.debateRoundComplete;
