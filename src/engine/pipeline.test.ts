...
import type { ModelRunResult } from "./model.js";
import { getPersonasFile, setPersonasFile } from "./personas.js";
import { HydraPipeline, type PipelineDependencies } from "./pipeline.js";
...
function createHarness(steps: ModelStep[]) {
	...
	const agents = new Map<
		string,
		ReturnType<PipelineDependencies["createAgentRun"]>
	>();
	const runFailures: string[] = [];
	const runCompletions: string[] = [];
	const addTokenUsageCalls: Array<
		Parameters<PipelineDependencies["addTokenUsage"]>
	> = [];
	...
	const deps: PipelineDependencies = {
		...
		createAgentRun: (
			input: Parameters<PipelineDependencies["createAgentRun"]>[0],
		) => {
...
describe("HydraPipeline", () => {
	test("successful run emits lifecycle events", async () => {
		...
		expect(
			harness.addTokenUsageCalls.reduce(
	...
	test("throws when all research agents fail", async () => {
		...
		await expect(pipeline.run("q")).rejects.toThrow(
			"research phase failed: 0/3 agents succeeded",
		);
	...
	test("sanitizes persona names in research failure logs", async () => {
		...
		try {
			...
			const pipeline = new HydraPipeline(createPipelineConfig(1, false, 1), {
				...harness.deps,
				personas: [unsafePersona],
			});
	...
	test("sanitizes upstream error payloads in research failure logs", async () => {
		...
		try {
			...
			expect(
				loggedLines.some((line) =>
					line.includes(
						"[hydra] research agent failed for Safe Persona: boomred next",
					),
				),
			).toBe(true);
		} finally {
	...
	test("fails debate round when fewer than two agents succeed", async () => {
		...
		await expect(pipeline.run("q")).rejects.toThrow(
			"debate round 1 failed: 1/3 agents succeeded",
		);
		expect(harness.runFailures.at(-1)).toBe(
			"debate round 1 failed: 1/3 agents succeeded",
		);
	...
	test("uses only custom personas when custom-personas-only is enabled and enough custom personas exist", async () => {
		...
		expect(harness.modelInputs[0]?.userPrompt).toContain(
			"- Custom Alpha: maps risk with constraints",
		);
		expect(harness.modelInputs[0]?.userPrompt).toContain(
			"- Custom Beta: finds execution details",
		);
	...
	test("generates ephemeral personas to fill custom-personas-only shortfall", async () => {
		...
		const harness = createHarness([
			...
			resolveStep(
				customDecomposeAssignmentsOutput([
					...customPersonas,
					...ephemeralPersonas,
				]),
			),
		...
		const pipeline = new HydraPipeline(
			createPipelineConfig(1, true, customPersonas.length + 1),
			harness.deps,
		);
		await pipeline.run("fill missing personas for query");

		expect(harness.modelInputs[0]).toBeDefined();
		expect(harness.modelInputs[0]?.systemPrompt).toBe(
		...
		expect(harness.modelInputs[1]?.userPrompt).toContain(
			"- Ephemeral Analyst: fills missing perspective",
		);
	});
