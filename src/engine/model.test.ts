...
import type { ModelRunInput } from "./model";

type CreateHandler = (params: unknown, options?: { signal?: AbortSignal }) => Promise<unknown>;

type TimerHandle = {
  cleared: boolean;
};

let createCalls: Array<{ params: unknown; options?: { signal?: AbortSignal } }> = [];
...
function interceptTimers(fireLongTimeout: boolean): {
  ...
  globalThis.setTimeout = ((callback: TimerHandler, ms?: number | undefined, ...args: unknown[]) => {
...
describe("runModelWithOptionalTools retry behavior", () => {
  ...
  test("per-call timeout fires and aborts hung requests", async () => {
    ...
    expect(createCalls).toHaveLength(5);
    expect(timers.delays.filter((delay) => delay === 120_000).length).toBe(createCalls.length);
  });
});
